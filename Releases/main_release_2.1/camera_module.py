...
MAX_IMAGES = 10   # keep only the newest N annotated images
# ==============================================================================
...
def detect_and_warp_mat(frame: np.ndarray):
    ...
    sorted_pts = contour_to_quad(mat_contour)
...
def open_camera():
    for idx in (CAMERA_INDEX, *CAMERA_FALLBACK_INDICES):
        ...
        # Force V4L2 explicitly - letting OpenCV auto-pick a backend on Pi OS
        # commonly selects GStreamer, whose plugin set is often incomplete for
        # UVC cameras and throws "Internal data stream error".
...
class CameraDimensioner:
    def __init__(self, output_dir=OUTPUT_DIR, max_images=MAX_IMAGES, on_capture=None):
    ...
    def _prune_old_images(self):
        files = sorted(
            (f for f in os.listdir(self.output_dir) if f.startswith("measured_result_")),
                ...
                if warped_mat is not None:
                    if state == ScanState.EMPTY:
                        if motion_score > MOTION_THRESHOLD and usable:
                            ...
                            print("[CameraDimensioner] Motion detected. Parcel entering mat...")
...
if __name__ == "__main__":
    print("=" * 60)
    print("       Standalone Camera Dimensioner Test Mode")
    print("=" * 60)
